...
ได้ครับ ไฟล์นี้ควรเป็นตัวคุม Skill + Platform + fallback resolution โดยเฉพาะกรณี skill version ไม่เข้ากัน, platform ไม่รองรับ, duplicate skills และ fallback chain เพราะ resolver คือจุดที่ตัดสินใจว่า Task จะเอาอะไรไปทำจริง
...
bash
pytest tests/unit/tasks/test_resolver.py -v
...
ชุดนี้ตั้งใจให้เป็น contract test ก่อนผูกกับ implementation จริง ดังนั้นเมื่อ core/tasks/resolver.py พร้อมแล้ว ควรเปลี่ยน FakeSkillResolver เป็น import ของ resolver จริง และคง test cases เหล่านี้ไว้เป็น regression suite

English: The resolver tests cover the important decision boundary between a parsed requirement and executable skills: existence, platform compatibility, multi-platform resolution, deduplication, disabled skills, and fallback selection. This is the layer that should prevent an Android-only skill from quietly being used for iOS, which is precisely the sort of tiny mistake that later becomes a three-hour debugging session.
